...
function fetchPageSource(prefetch, url) {
    return new Promise((resolve, reject) => {
        console.log(
            `[${chalk.blue(new Date().toLocaleTimeString('vi'))}] ${
                prefetch ? 'Prefetching' : 'Fetching'
...
function parsePageSource(raw) {
    return new Promise((resolve, reject) => {
        console.log(`[${chalk.blue(new Date().toLocaleTimeString('vi'))}] Parsing page source... ${estimate()}`);
...
function downloadVideo({ name, url }) {
    return new Promise((resolve, reject) => {
        console.log(`[${chalk.blue(new Date().toLocaleTimeString('vi'))}] Downloading video... ${estimate()}`);
...
function saveVideo({ where, buffer }) {
    return new Promise((resolve, reject) => {
        console.log(`[${chalk.blue(new Date().toLocaleTimeString('vi'))}] Saving video... ${estimate()}`);
        fs.writeFileSync(path.join(config.saveDir, `${where}.tido.mp4`), buffer);
        console.log(`[${chalk.blue(new Date().toLocaleTimeString('vi'))}] Video saved! ${estimate()}`);
    });
}
